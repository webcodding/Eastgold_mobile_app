...
const WaiterCard = ({
  ...
  const matchedManagers = managers.filter(
    (man) => man.id === parseInt(item.branch_manager_id)
  );
  ...
  return (
    <TouchableOpacity
      ...
      onPress={() =>
        navigation.navigate("WaiterDetail", {
          ...
          manager: matchedManagers.length > 0 ? matchedManagers[0] : [],
          token: token,
          profile: profile,
        })
      }
    >
      ...
      <Text style={[golbalStyle.normalText, styles.name]}>{item.name}</Text>
      ...
      >
        ...
        {/* <View style={[golbalStyle.row, { marginTop: 0 }]}>
          <Image source={GoldCoin} style={{ width: 20, height: 20 }} />
          <Text
            style={[golbalStyle.normalText, { opacity: 0.9, marginTop: 3 }]}
          >
            {item.gold_back}%
          </Text>
        </View> */}
